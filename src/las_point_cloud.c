...
int analyze_sources(const struct LasPoint *points, size_t point_num,
  ...
  for (int i = 0; i < source_num; ++i) {
    ...
    printf("Source %d: min_angle %d max_angle %d repr_angle	%d\n", i,
           s.min_angle, s.max_angle, s.repr_angle);
  }
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < source_num; ++i) {
    ...
    printf("Source %d has azimuth %4d and negative angle is %s\n",
           source.source_id, (int)(source.azimuth * 180 / M_PI),
  ...
  using KDTree = nanoflann::KDTreeSingleIndexAdaptor<
      nanoflann::L2_Simple_Adaptor<float, PointCloudPos>, PointCloudPos, 3>;
  ...
  do {
    ...
    for (size_t i = 0; i < filtered_num; i++) {
      ...
      Vec3 query_pt = coords[i];
      tree.knnSearch(&query_pt[0], num_probes, &ret_index[0], &out_dist_sqr[0]);
      F32 majority = 0;
      for (int j = 0; j < num_probes; ++j) {
        int k = ret_index[j];
        if (settled[k]) {
  ...
  file.add_properties_to_element(
      "vertex", {"x", "y", "z"}, tinyply::Type::FLOAT32, npoints,
      reinterpret_cast<U08 *>(coords), tinyply::Type::INVALID, 0);
  file.add_properties_to_element(
      "vertex", {"nx", "ny", "nz"}, tinyply::Type::FLOAT32, npoints,
      reinterpret_cast<U08 *>(normals), tinyply::Type::INVALID, 0);
  file.add_properties_to_element("vertex", {"quality"}, tinyply::Type::FLOAT32,
                                 npoints, reinterpret_cast<U08 *>(quality),
