...
{
struct arithmetic {
	arithmetic(uint32_t syms, bool com = false,
		   uint32_t *initTable = nullptr)
	    : symbols(syms), compress(com), distribution(nullptr),
	      symbol_count(nullptr), decoder_table(nullptr)
	{
		...
		if ((!compress) && (symbols > 16)) {
			...
			decoder_table =
			    reinterpret_cast<uint32_t *>(utils::aligned_malloc(
				sizeof(uint32_t) * (table_size + 2)));
		} else { // small alphabet: no table needed
			decoder_table = 0;
			table_size = table_shift = 0;
		}

		distribution = reinterpret_cast<uint32_t *>(
		    utils::aligned_malloc(symbols * sizeof(uint32_t)));
		symbol_count = reinterpret_cast<uint32_t *>(
		    utils::aligned_malloc(symbols * sizeof(uint32_t)));
	...
	arithmetic(const arithmetic &other)
	    : symbols(other.symbols), compress(other.compress),
	      ...
	      last_symbol(other.last_symbol), table_size(other.table_size),
	      table_shift(other.table_shift)
	{
		size_t size(symbols * sizeof(uint32_t));
		distribution =
		    reinterpret_cast<uint32_t *>(utils::aligned_malloc(size));
		std::copy(other.distribution, other.distribution + symbols,
			  distribution);

		symbol_count =
		    reinterpret_cast<uint32_t *>(utils::aligned_malloc(size));
		std::copy(other.symbol_count, other.symbol_count + symbols,
			  symbol_count);

		if (table_size) {
			size = (table_size + 2) * sizeof(uint32_t);
			decoder_table = reinterpret_cast<uint32_t *>(
			    utils::aligned_malloc(size));
			std::copy(other.decoder_table,
				  other.decoder_table + (table_size + 2),
				  decoder_table);
		} else
	...
	arithmetic(arithmetic &&other)
	    : symbols(other.symbols), compress(other.compress),
	      ...
	      table_shift(other.table_shift)
	{
		other.distribution = other.decoder_table = other.symbol_count =
		    NULL;
	...
	arithmetic &operator=(arithmetic &&other)
	{
		if (this != &other) {
			...
			other.distribution = other.symbol_count =
			    other.decoder_table = nullptr;
			other.total_count = other.update_cycle =
			    other.symbols_until_update = other.last_symbol =
				other.table_size = other.table_shift = 0;
		}
...
struct arithmetic_bit {
	...
	arithmetic_bit(arithmetic_bit &&other)
	    : update_cycle(other.update_cycle),
	      ...
	      bit_0_prob(other.bit_0_prob), bit_0_count(other.bit_0_count),
	...
	arithmetic_bit &operator=(arithmetic_bit &&other)
	{
		if (this != &other) {
			...
			other.update_cycle = other.bits_until_update =
			    other.bit_0_prob = other.bit_0_count =
				other.bit_count = 0;
		}
