...
{
struct integer {
	integer(uint32_t bits = 16, uint32_t contexts = 1,
		uint32_t bits_high = 8, uint32_t range = 0)
	...
	template <typename TDecoder, typename TEntroyModel>
	int32_t readCorrector(TDecoder &dec, TEntroyModel &mBits)
	{
		...
		// decode the exact location of the corrector within the
		// interval
		...
		if (k) // then c is either smaller than 0 or bigger than 1
		{
			if (k < 32) {
				...
				if (c >=
				    (1 << (k - 1))) // if c is in the interval [
						    // 2^(k-1)  ...  + 2^k - 1 ]
				{
					// so we translate c back into the
					// interval [ 2^(k-1) + 1  ...  2^k ] by
					// adding 1
					c += 1;
				} else // otherwise c is in the interval [ 0 ...
				       // + 2^(k-1) - 1 ]
				{
					// so we translate c back into the
					// interval [ - (2^k - 1)  ...  -
		...
		{
			if (k < 32) {
				if (k <= bits_high) // for small k we can do
						    // this in one step
				{
					// decompress c with the range coder
					c = dec.decodeSymbol(mCorrector[k - 1]);
				} else {
					// for larger k we need to do this in
					// two steps
					int k1 = k - bits_high;
