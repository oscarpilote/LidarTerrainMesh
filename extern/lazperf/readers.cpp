...
struct basic_file::Private {
	Private()
	    : head12(head14), head13(head14), compressed(false),
	...
	void parseVLRs();
	bool extractVlr(const std::string &user_id, uint16_t record_id,
			uint64_t data_length);
	std::vector<char> vlrData(const std::string &user_id,
				  uint16_t record_id);
...
{
	f = &in;
	// ABELL - move to loadHeader() in order to avoid the reset on
	// InFileStream.
...
{
	// There is a chunk offset where the first point is supposed to be. The
	// first chunk follows that.
...
{
	if (!compressed)
		stream->cb()(reinterpret_cast<unsigned char *>(out),
			     head12.point_record_length);

	// read the next point
	else {
		if (!pdecompressor || chunk_point_num == current_chunk->count) {
			pdecompressor = build_las_decompressor(
			    stream->cb(), head12.point_format_id,
			    head12.ebCount());
...
bool basic_file::Private::loadHeader()
{
	...
	// may have treaded past the EOL, so reset everything before we start
	// reading
	f->clear();
...
void basic_file::Private::parseVLRs()
{
	...
	if (head14.evlr_count && head14.evlr_offset != 0) {
		...
		while (count < head14.evlr_count && f->good() && !f->eof()) {
			...
			// If we don't read the VLR, seek past it.
			if (!extractVlr(h.user_id, h.record_id, h.data_length))
				f->seekg(h.data_length,
					 std::ios::cur); // jump forward
			count++;
		}
	}
...
bool basic_file::Private::extractVlr(const std::string &user_id,
				     uint16_t record_id, uint64_t data_length)
{
	// Extract LASzip VLR
	if (user_id == "laszip encoded" && record_id == 22204) {
		...
		if ((head12.pointFormat() <= 5 && laz.compressor != 2) ||
			...
			throw error(
			    std::string("Mismatch between point format of ") +
			    std::to_string(head12.pointFormat()) +
			    " and compressor version of " +
...
std::vector<char> basic_file::Private::vlrData(const std::string &user_id,
					       uint16_t record_id)
{
	std::vector<char> data;

	// Go through the VLR index looking for the one we want. If we find it,
	// seek to the data location, read the data into the vector, then seek
	// pack where we were.
...
void basic_file::Private::parseChunkTable()
{
	...
	if (chunkoffset == -1)
		throw error(
		    "Chunk table offset == -1 is not supported at this time");
...
#pragma pack(pop)
	...
	// Allocate enough room for the chunk table plus one because of the
	// crazy way that the chunk table is written. Once it is fixed up, we
	// resize back to the correct size.
	...
	// This discards the last offset, which we don't care about. The last
	// count is never filled in.
...
void basic_file::Private::validateHeader()
{
	...
	if (bit_7 == 1 && bit_6 == 1)
		throw error(
		    "Header bits indicate unsupported old-style compression.");
	if ((bit_7 ^ bit_6) == 0)
...
bool basic_file::open(std::istream &f) { return p_->open(f); }

void basic_file::readPoint(char *out) { p_->readPoint(out); }

const header14 &basic_file::header() const { return p_->head14; }

uint64_t basic_file::pointCount() const { return p_->pointCount(); }

laz_vlr basic_file::lazVlr() const { return p_->laz; }

std::vector<char> basic_file::vlrData(const std::string &user_id,
				      uint16_t record_id)
{
...
chunk_decompressor::chunk_decompressor(int format, int ebCount,
				       const char *srcbuf)
    : p_(new Private)
{
