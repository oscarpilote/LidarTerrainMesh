...
// John McDonald, Mark Kilgard. Crack-Free Point-Normal Triangles using Adjacent
// Edge Normals. 2010
namespace meshopt
{

static unsigned int hashUpdate4(unsigned int h, const unsigned char *key,
				size_t len)
{
	// MurmurHash2
...
struct VertexHasher {
	...
	{
		return hashUpdate4(0, vertices + index * vertex_stride,
				   vertex_size);
	}

	bool equal(unsigned int lhs, unsigned int rhs) const
	{
		return memcmp(vertices + lhs * vertex_stride,
			      vertices + rhs * vertex_stride, vertex_size) == 0;
	}
};
...
struct VertexStreamHasher {
	...
	bool equal(unsigned int lhs, unsigned int rhs) const
	{
		for (size_t i = 0; i < stream_count; ++i) {
			...
			if (memcmp(data + lhs * s.stride, data + rhs * s.stride,
				   s.size) != 0)
				return false;
		}
...
static T *hashLookup(T *table, size_t buckets, const Hash &hash, const T &key,
		     const T &empty)
{
	assert(buckets > 0);
...
static void buildPositionRemap(unsigned int *remap,
			       const float *vertex_positions,
			       size_t vertex_count,
			       size_t vertex_positions_stride,
			       meshopt_Allocator &allocator)
{
	VertexHasher vertex_hasher = {
	    reinterpret_cast<const unsigned char *>(vertex_positions),
	    3 * sizeof(float), vertex_positions_stride};

	size_t vertex_table_size = hashBuckets(vertex_count);
	unsigned int *vertex_table =
	    allocator.allocate<unsigned int>(vertex_table_size);
	memset(vertex_table, -1, vertex_table_size * sizeof(unsigned int));

	for (size_t i = 0; i < vertex_count; ++i) {
		unsigned int index = unsigned(i);
		unsigned int *entry = hashLookup(
		    vertex_table, vertex_table_size, vertex_hasher, index, ~0u);
				   ...
				   size_t vertex_count, size_t vertex_size)
{
	...
	for (size_t i = 0; i < index_count; ++i) {
		...
		if (destination[index] == ~0u) {
			unsigned int *entry =
			    hashLookup(table, table_size, hasher, index, ~0u);
...
size_t meshopt_generateVertexRemapMulti(unsigned int *destination,
					const unsigned int *indices,
...
void meshopt_remapVertexBuffer(void *destination, const void *vertices,
			       size_t vertex_count, size_t vertex_size,
			       const unsigned int *remap)
{
	assert(vertex_size > 0 && vertex_size <= 256);
	...
	for (size_t i = 0; i < vertex_count; ++i) {
		if (remap[i] != ~0u) {
			...
			memcpy(static_cast<unsigned char *>(destination) +
				   remap[i] * vertex_size,
...
void meshopt_remapIndexBuffer(unsigned int *destination,
...
void meshopt_generateShadowIndexBuffer(unsigned int *destination,
				       const unsigned int *indices,
				       size_t index_count, const void *vertices,
				       size_t vertex_count, size_t vertex_size,
				       size_t vertex_stride)
...
void meshopt_generateShadowIndexBufferMulti(
    ...
    size_t vertex_count, const struct meshopt_Stream *streams,
    size_t stream_count)
{
	using namespace meshopt;
...
void meshopt_generateAdjacencyIndexBuffer(unsigned int *destination,
					  const unsigned int *indices,
					  size_t index_count,
					  const float *vertex_positions,
					  size_t vertex_count,
					  size_t vertex_positions_stride)
{
	...
	// build position remap: for each vertex, which other (canonical) vertex
	// does it map to?
	unsigned int *remap = allocator.allocate<unsigned int>(vertex_count);
	buildPositionRemap(remap, vertex_positions, vertex_count,
			   vertex_positions_stride, allocator);

	// build edge set; this stores all triangle edges but we can look these
	// up by any other wedge
	EdgeHasher edge_hasher = {remap};

	size_t edge_table_size = hashBuckets(index_count);
	unsigned long long *edge_table =
	    allocator.allocate<unsigned long long>(edge_table_size);
	unsigned int *edge_vertex_table =
	    allocator.allocate<unsigned int>(edge_table_size);
	...
	for (size_t i = 0; i < index_count; i += 3) {
		for (int e = 0; e < 3; ++e) {
			...
			unsigned long long edge =
			    ((unsigned long long)i0 << 32) | i1;
			unsigned long long *entry =
			    hashLookup(edge_table, edge_table_size, edge_hasher,
				       edge, ~0ull);
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		for (int e = 0; e < 3; ++e) {
			...
			patch[e * 2 + 1] =
			    (*oppe == ~0ull)
				? i0
				: edge_vertex_table[oppe - edge_table];
		}
...
void meshopt_generateTessellationIndexBuffer(unsigned int *destination,
					     const unsigned int *indices,
					     size_t index_count,
					     const float *vertex_positions,
					     size_t vertex_count,
	...
	for (size_t i = 0; i < index_count; i += 3) {
		...
		for (int e = 0; e < 3; ++e) {
			...
			unsigned long long edge =
			    ((unsigned long long)i1 << 32) | i0;
			unsigned long long oppe =
			    *hashLookup(edge_table, edge_table_size,
					edge_hasher, edge, ~0ull);

			// use the same edge if opposite edge doesn't exist
			// (border)
